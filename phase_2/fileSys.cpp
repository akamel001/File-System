...
fileSys::fileSys(Pdisk& dsk):disk(dsk){
...
void fileSys::init(){
	int num_blks = disk.getNumOfBlocks(), tmp;
	
	//(num_blks * (floor(log10( num_blks ))+2)) / disk.getBlockSize();
	fatSize = ceil(disk.getBlockSize() / disk.getNumOfBlocks());
	
	//	# of char in filename + 1 space + # of digits in blk number + 1 + 1 space
	tmp = 8 + 1 + floor( log10( disk.getNumOfBlocks() ) ) + 1 + 1; 
	rootSize = disk.getBlockSize() / tmp;
...
void fileSys::make_fs(){
	...
	for (int i = 1; i < fat.size()-2;i++)
		fat[ fat[0]+ (i-1) ] = fat[0] + i;
	fat[fat.size()-1] = 0;	
...
int fileSys::check_block(int block, string file){
   ...
   if(found == false){
      cout << "getBlock(): Hey! That block number does not belong to that file." << endl;
      return 0;
   }
	return 1;
}
